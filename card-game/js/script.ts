...
function updateCount() {
  ...
  //   if (leftElement !== null) {
  //   leftElement.innerHTML = `${deck.length + 1} kort kvar`;}
}
...
function lower(e) {
  if (deck.length > 0 && attempts > 1) {
    ...
    setTimeout(() => {
      ;(document.querySelector('#gameover') as HTMLElement).classList.toggle(
        'show'
      )
    }, 600)
  }
...
function higher(e) {
  if (deck.length > 0 && attempts > 1) {
    ...
    setTimeout(() => {
      const HTMLElement = document.querySelector('#gameover')
      if (HTMLElement) {
        if (HTMLElement !== null) {
          HTMLElement.classList.toggle('show')
        }
      }
    }, 600)
  }
